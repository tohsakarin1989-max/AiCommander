...
历史 v4.5 实现工作包 `41653a6` 已完成连续五轮技术业务彩排。当前 v5.3 增加持续专题、全库条件统计与同版周期材料，见 [v5.3 发布说明](docs/releases/v5.3.0-stable.md)；v5.0—v5.2 已有能力见 [v5.2 发布说明](docs/releases/v5.2.0-stable.md)。GitHub Release 和目标服务器部署分别核实，不以源码版本号代替。
...
当前源码版本为 **v5.3.0-stable**，在日常业务整合、统一历史检索、道路与生产条件融合研判基础上增加专题研判与周期材料。
升级必须使用同时具备 PostGIS 和 pgvector 的数据库镜像，不可直接沿用旧纯 PostGIS 默认镜像。
功能、验证边界、迁移和保护新增原始数据的回退步骤见 [v5.3 发布说明](docs/releases/v5.3.0-stable.md)。

...
历史 v4.5 实现工作包 `41653a6` 已完成连续五轮技术业务彩排。当前 v5.4 贯通重点设施综合档案、全域历史条件对照和统一地图时间线，见 [v5.4 发布说明](docs/releases/v5.4.0-stable.md)；持续专题见 [v5.3 发布说明](docs/releases/v5.3.0-stable.md)，v5.0—v5.2 能力见 [v5.2 发布说明](docs/releases/v5.2.0-stable.md)。GitHub Release 和目标服务器部署分别核实，不以源码版本号代替。
...
当前源码版本为 **v5.4.0-stable**，在案件、历史检索、道路研判与专题材料基础上增加设施综合档案、区域条件对照及共享地图时间范围；明确关联、空间邻近与候选分开，不新增风险评分或执行任务。
升级必须使用同时具备 PostGIS 和 pgvector 的数据库镜像，不可直接沿用旧纯 PostGIS 默认镜像。
功能、验证边界、迁移和保护新增原始数据的回退步骤见 [v5.4 发布说明](docs/releases/v5.4.0-stable.md)。

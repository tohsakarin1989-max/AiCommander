...
v2.3 在保留地图数据管家能力的基础上，新增案件保存前服务端规则预检、指定人员案件数据管家和
案件页受控批量质检入口。案件数据管家始终只读，必须明确选择案件，单次默认最多 30 起；它只生成
质量问题和证据索引，不生成案件字段写入操作。系统默认仍不需要 OpenAI 或其他模型 API。
...
当前 `v2.3.0-stable` 回归基线：后端 253 项测试、前端 21 个测试文件共 89 项测试通过。

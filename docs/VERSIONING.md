...
- Release branch: `codex/v2.9.0-stable`
- Release tag: `v2.9.0-stable` on `main` after review and merge.
...
- Purpose: stable code baseline for a read-only, single-case evidence corridor that connects case fields, registered evidence metadata, map/well context, chain hypotheses, Agent artifacts, and versioned knowledge assets. Broken or unresolved references are surfaced for review; spatial proximity and inferred relationships remain explicitly non-factual. The daily workbench, both data stewards, and dual-domain analysis remain available, while Agent Lab and external models stay disabled by default.

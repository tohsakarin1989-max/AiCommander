...
> 当前 v5.4 源码升级须优先阅读 [v5.4 发布、迁移与回退说明](./releases/v5.4.0-stable.md)。专题及设施目录摘要由普通 Celery Worker 与 Beat 处理，不要求启用 Agent Lab。
> v5.1 起数据库必须同时含 PostGIS 和 pgvector；当前配置不再默认沿用本手册历史章节的纯 PostGIS 镜像。
> 使用相容备份恢复到独立新卷并验证后切换，不直接给旧数据卷更换基础发行版。
> 以下 v3.6 及更早章节保留为历史部署证据，不代表 v5.4 已在目标服务器投产。
...
### 1.3 2026-09-07 v2.3.0-stable 代码级验证

| 检查项 | 结果 |
| --- | --- |
| 后端测试 | 253 项全部通过 |
| 前端测试 | 21 个测试文件、89 项全部通过 |
| TypeScript 类型检查 | 通过 |
| 前端生产构建 | 通过 |
| 案件保存前服务端预检 | 只读返回评分、缺项和提醒；不创建或修改案件 |
| 案件数据管家 | 指定人员、显式案件范围、单次最多 30 起、任何模式下禁止案件写入 |
| 真实浏览器链路 | 保存前预检、人工确认、只读质检、证据轨迹和一键停用全部通过 |
| 隔离生产部署 | 五服务健康、全量迁移、Agent 默认关闭、自动冒烟和备份恢复全部通过 |

上述是源码发布提交的代码级结果和本机隔离生产演练结果。目标服务器部署、单位网络、真实业务数据和
业务人员验收仍应分别留存证据，未完成前不得表述为正式投产。

### 1.4 2026-09-07 v2.4.0-stable 代码级验证

| 检查项 | 结果 |
| --- | --- |
| 后端测试 | 265 项全部通过；Agent 专项回归 113 项全部通过 |
| 前端测试 | 22 个测试文件、93 项全部通过；Agent 专项 6 个文件、23 项全部通过 |
| TypeScript 与生产构建 | 通过；仅有既有的大分块体积提示 |
| 生产依赖审计 | Python 和前端生产依赖均未发现已知漏洞 |
| SQLite/PostgreSQL 空库迁移 | 均到达 `c3a8d4f2b711` |
| 双域浏览器链路 | 指定人员、双范围选择、空间关联、历史热点、证据轨迹和一键停用全部通过 |
| 只读边界 | 2 条证据引用、0 个待审批；停用后重放被明确拒绝，正式数据不变 |
| 隔离生产部署 | 五服务健康、Agent 默认关闭、自动冒烟、备份校验和临时库恢复全部通过 |
| 临时资源清理 | 演练容器全部清理 |

上述结果证明 v2.4 发布代码具备前期测试部署条件，不代表已经完成目标服务器和真实业务现场验收。

### 1.5 2026-09-07 v2.5.0-stable 代码级验证

| 检查项 | 结果 |
| --- | --- |
| 后端测试 | 273 项全部通过；Agent 专项回归 121 项全部通过 |
| 前端测试 | 22 个测试文件、94 项全部通过；Agent 专项 6 个文件、24 项全部通过 |
| TypeScript 与生产构建 | 通过；仅有既有的大分块体积提示 |
| 生产依赖审计 | Python 和前端生产依赖均未发现已知漏洞 |
| SQLite/PostgreSQL 空库迁移 | 均到达 `e4b5c6d7a812` |
| 统一运行中心浏览器链路 | 空态、规则任务、30 天汇总、运行详情、版本与网络请求全部通过；新增请求无控制台错误 |
| 隔离生产部署 | 五服务健康、Agent 默认关闭、自动冒烟、备份校验和临时库恢复全部通过 |
| 备份恢复 | 临时库恢复 40 张表，迁移版本 `e4b5c6d7a812` |
| 临时资源清理 | 演练容器、网络、数据卷和构建镜像全部清理 |

上述结果证明 v2.5 发布代码具备前期测试部署条件。没有调用真实模型，也没有代替目标服务器、单位网络、
真实业务数据和业务人员验收；外部/内网模型的连通、单价与账单核对仍需现场留痕。

### 1.6 2026-09-08 v2.6.0-stable 代码级验证

| 检查项 | 结果 |
| --- | --- |
| 后端测试 | 285 项全部通过；知识资产专项 38 项、Agent 专项 121 项全部通过 |
| 前端测试 | 22 个测试文件、95 项全部通过；知识资产呈现 5 项、Agent 专项 6 个文件、24 项全部通过 |
| TypeScript 与生产构建 | 通过；仅有既有的大分块体积提示 |
| 生产依赖审计 | Python 和前端生产依赖均未发现已知漏洞 |
| SQLite/PostgreSQL 空库迁移 | 均到达 `f6c8d2e4a913` |
| 知识资产浏览器链路 | 经验卡保存/确认、历史推荐、采纳、报告引用、报告确认和复用轨迹全部通过 |
| 隔离生产部署 | 五服务健康、Agent 默认关闭、自动冒烟、备份校验和临时库恢复全部通过 |
| 备份恢复 | 临时库恢复 42 张表，迁移版本 `f6c8d2e4a913` |
| 网络与认证 | 仅前端绑定 `127.0.0.1:33080`；匿名案件请求 401；接口文档关闭 |
...
### 1.8 2026-09-08 v2.9.0-stable 代码级验证

| 检查项 | 结果 |
| --- | --- |
| 后端测试 | 303 项全部通过；证据图谱专项 7 项全部通过 |
| 前端测试 | 24 个测试文件、104 项全部通过；证据图谱呈现 4 项全部通过 |
| TypeScript 与生产构建 | 通过；仅有既有的大分块体积提示 |
| 生产依赖审计 | Python 生产依赖解析与前端生产依赖均未发现已知漏洞 |
| SQLite/PostgreSQL 空库迁移 | 均到达 `a7d9e1f2b304`；v2.9 没有新增数据库表 |
| 真实浏览器链路 | 案件导航、工作台入口、六层证据走廊、断链队列、版本信息和站内 GET 请求通过 |
| 数据边界 | 材料路径/正文、人员身份、电话、证件号、车牌和精确井点坐标未进入图谱响应 |
| 隔离生产部署 | 五服务健康、Agent 默认关闭、自动冒烟、内部端口限制和 43 表备份恢复通过 |

上述结果仅证明发布代码具备前期测试部署条件。目标服务器备份与迁移、单位网络与 HTTPS、真实案件
证据适用性及业务人员签字仍需在部署现场完成。

### 1.9 2026-09-08 v3.0.0-stable 代码级验证

| 检查项 | 结果 |
| --- | --- |
| 后端测试 | 315 项全部通过；双域态势专项 12 项全部通过 |
| 前端测试 | 25 个测试文件、109 项全部通过；态势呈现 5 项全部通过 |
| TypeScript 与生产构建 | 通过；仅有既有的大分块体积提示 |
| 生产依赖审计 | Python 生产依赖解析与前端生产依赖均未发现已知漏洞 |
| SQLite/PostgreSQL 空库迁移 | 均到达 `a7d9e1f2b304`；v3.0 没有新增数据库表 |
| 真实浏览器链路 | 登录、首页与研判导航、默认 30 天窗口、核查重点、态势沙盘、井点排序、版本号和站内 GET 请求通过 |
| 数据边界 | 人员、电话、证件号、车牌和案件描述全文未进入态势响应；外部模型未启用 |
| 隔离生产部署 | 五服务健康、Agent 默认关闭、自动冒烟、内部端口限制和 43 表备份恢复通过 |

上述结果仅证明发布代码具备前期测试部署条件。目标服务器备份迁移、单位网络与 HTTPS、真实案件态势
适用性、节时效果和业务人员签字仍需在部署现场完成。

### 1.10 v3.0 继承并扩展的生产保护
...
- 案件受控辅助只允许指定管理员或分析员发起、必须明确选择现有案件，单次默认最多 30 起。
- 案件数据管家始终只读，不生成或批准案件字段写入；管理员可一键停用并取消活动案件任务。
- 双域融合研判只允许指定管理员或分析员发起，必须同时明确选择案件和有效地图资源；默认单次最多
  10 起案件、100 项资源，计算不得越过所选范围。
- 双域分析和综合证据报告始终只读，只复盘历史时空条件；不形成犯罪预测、自动串并案结论或巡逻指令。
- 管理员可独立一键停用双域试用；活动双域任务和综合报告取消，已有证据轨迹保留，正式数据不变。
- 保存前服务端预检不依赖 Agent Worker、Redis 或外部模型；异常时允许人工确认后继续走核心保存链路。
- 经验卡和案件研判报告采用独立版本；源数据或报告统计范围变化后，旧草稿不能被确认。
- 只有已人工确认且未归档的历史经验可以被推荐；采纳、排除和报告引用均保留复用轨迹。
- 历史经验只进入报告参考章节，不自动修改案件、复制旧案结论或生成处置任务。
- 统一运行中心只保存提供方、模型名、耗时、Token、估算成本和错误类型，不保存提示词、响应正文、密钥或网关地址。
- 可选模型可以使用 OpenAI Agents 适配器或现有模型注册表；默认 `deterministic`，不强制使用 OpenAI API。
- 模型注册表只能按部署配置选择已启用模型，普通用户不能逐任务任意切换；模型失败时内网规则结果保留。
- 单案证据图谱只有读取接口；图谱失败不影响案件、地图、报告、知识资产和 Agent 主流程。
- 图谱不读取或返回材料文件路径与正文；人员、电话、证件号和车牌仅用于遮蔽自由文本误填内容，不作为节点或详情返回。
- 井点只输出限定半径内的距离和核验状态，不输出精确坐标；空间接近始终标记为参考条件。
- Agent 成果或知识资产的任一失效引用都会进入断链复核队列，不得包装为完整证据路径。
- 双域态势研判只有读取接口，比较相邻历史窗口，不依赖 Agent Worker 或外部模型。
- 态势响应不查询人员、电话、证件号、车牌和案件描述全文，只读取完成计算所需的案件及井点字段。
- 当前窗口无案件时不生成热点、井点排序或泛化建议；有数据时最多突出三项核查重点。
- 历史热点、手法变化和井点空间接近只作为人工核查入口，不形成犯罪预测、串案结论或巡逻任务。
...
| --- | ---: | ---: | ---: | --- |
| 测试服务器 | 4 核 | 8 GB | 80 GB SSD | 少量并发，默认使用内网规则引擎 |
| 单机生产建议 | 8 核 | 16 GB | 160 GB SSD | 适合当前单后端 worker 架构 |
| 数据量或并发增长 | 16 核 | 32 GB | 300 GB SSD | 需同步做压测和数据库监控 |

至少为 `/var/lib/docker`、`/opt/aicommander/backups` 和系统日志预留空间。Agent Lab 的
规则能力可在断网内网运行；只有经安全评审后启用可选模型适配器时，才需要相应模型服务。
...
回退前先停止业务写入和后台任务，并另外保存**升级后的数据库备份**、对应地图文件卷、
地图版本清单、配置和原有加密密钥。不得直接恢复旧备份而丢弃升级后录入的原始案件。
优先在另一数据库和独立地图目录恢复上一Stable兼容备份，旧程序只连接兼容恢复库；
升级后的原库及其备份保持受控保留。恢复旧版不代表升级后的新增数据已经迁回，后续须核对后迁移，
不可直接拼接不同版本表或自动覆盖旧记录。

2026-09-10已完成v3.6旧源码、当前源码及PostgreSQL带合成数据的升级与另库恢复验证，
见[v4.0升级恢复记录](validation/v4.0-upgrade-restore.md)。这不替代地图文件卷、完整镜像和现场恢复验收。

...
describe('suggestionPresentation', () => {
  ...
  it('routes only to low-risk review and navigation surfaces', () => {
    ...
    expect(getSuggestionRoute(suggestion({ action: 'review_experience_card', target_id: 7 }))).toBe('/case-intelligence?caseId=7&tool=experience')
    expect(getSuggestionRoute(suggestion({ action: 'review_processing_card', target_id: 188 }))).toBe('/cases?caseId=188')
    expect(getSuggestionRoute(suggestion({ action: 'preprocess_case', target_id: 188 }))).toBe('/cases?caseId=188')
  ...
  it('builds a review detail panel with fact inference suggestion and boundary sections', () => {
    ...
    expect(detail.inferences.join(' ')).not.toContain('确认后才能进入下一步')
    expect(detail.suggestions.join(' ')).toContain('可先正常使用案件')
  })
})

...
class CaseStewardControlUpdate(BaseModel):
    enabled: bool
    pilot_user_ids: list[int] = Field(default_factory=list, max_length=50)
    reason: str = Field(..., min_length=2, max_length=500)


class CaseStewardSuspendRequest(BaseModel):
    reason: str = Field(..., min_length=2, max_length=500)
...
def _require_access(request: Request, *, admin_only: bool = False):
    if not settings.ENABLE_AGENT_LAB or settings.AGENT_MODE == "off":
        raise HTTPException(status_code=404, detail="案件数据管家未启用")
    principal = _principal(request)
    role = getattr(principal, "role", "admin" if not settings.AUTH_REQUIRED else None)
    if role not in {"admin", "analyst"}:
        raise HTTPException(status_code=403, detail="当前账号无权访问案件数据管家")
...
def _status(db: Session, principal) -> dict[str, Any]:
    return CaseStewardPilotService.build_status(
        db,
...
@router.put("/control")
def update_case_steward_control(
    payload: CaseStewardControlUpdate,
    request: Request,
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    principal = _require_access(request, admin_only=True)
    if payload.enabled and settings.AGENT_MODE != "assist":
        raise HTTPException(status_code=409, detail="案件数据管家试用必须在受控辅助模式开启")
    try:
    ...
    if not payload.enabled:
        AgentRunService.suspend_case_pilot(
            db,
...
@router.post("/suspend")
def suspend_case_steward(
    payload: CaseStewardSuspendRequest,
    request: Request,
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    principal = _require_access(request, admin_only=True)
    current = CaseStewardPilotService.get_control(db)
    CaseStewardPilotService.set_control(
        db,
        enabled=False,

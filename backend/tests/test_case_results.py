...
def test_catalog_search_pagination_and_hidden_scope(db_session, result_data):
    prepare(db_session)
    first, _ = CaseResultService.create_current(db_session, 1)
    db_session.add(CaseAnalysisProfile(id="p-hidden", case_id=2, profile_version=1, source_hash="hidden",
        schema_version="4.1.0", dictionary_version="rules-1", payload={"source_hash": "hidden"},
        quality_score=1, analysis_readiness="ready"))
    db_session.commit()
    db_session.info["authorized_area_ids"] = None
    CaseResultService.create_current(db_session, 2)
    db_session.commit()
    with client_for(db_session) as client:
        catalog = client.get("/api/case-results?limit=1").json()
        assert catalog["has_more"] and len(catalog["items"]) == 1
        assert not client.get("/api/case-results?limit=1&offset=1").json()["has_more"]
        db_session.info["authorized_area_ids"] = (1,)
        response = client.get("/api/case-results?q=RESULT-1")
        assert response.status_code == 200
        assert response.headers["cache-control"] == "no-store"
        assert response.json()["items"][0]["id"] == first["id"]
        assert client.get("/api/case-results?q=RESULT-2").json()["items"] == []
        assert client.get("/api/case-results?q=%25").json()["items"] == []
        assert client.get("/api/case-results?limit=101").status_code == 422
        assert client.get("/api/case-results", params={"q": "x" * 101}).status_code == 422


def test_catalog_keeps_revoked_evidence_out_of_metadata(db_session, result_data):
    prepare(db_session)
    ...
    db_session.info["authorized_area_ids"] = (1,)
    with client_for(db_session) as client:
        response = client.get("/api/case-results")
        assert response.json()["items"] == [{"id": saved["id"], "case_id": 1, "case_number": "RESULT-1", "availability": "unavailable"}]
        assert "测试候选" not in response.text and "case:2" not in response.text
...
def test_real_login_and_persisted_scope_revocation(db_session, result_data):
    ...
    with TestClient(app) as client:
        assert client.get("/api/cases/1/results/history").status_code == 401
        assert client.get("/api/case-results").status_code == 401
        ...
        assert len(client.get("/api/case-results").json()["items"]) == 1
        with factory() as db:
        ...
        assert client.get("/api/case-results").json()["items"] == []
        assert client.get("/api/cases/1/results/history").status_code == 404
